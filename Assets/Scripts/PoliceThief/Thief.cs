using MLAgents;
using UnityEngine;

namespace PoliceThief
{
    public class Thief : Agent
    {
//        public float[,] belief;
//        public float initialProb = 0.5f;

//        public int enemyIndex;
        private World world;

        private Rigidbody rigidBody;

        private Character character;
        private void Awake()
        {
            character = GetComponent<Character>();
            rigidBody = GetComponent<Rigidbody>();
            world = transform.parent.GetComponent<World>();
        }

        public override void InitializeAgent()
        {

        }

        public override void AgentReset()
        {
            Debug.Log("reset");
            character.RandomResetPosition();;
        }

        public override void AgentAction(float[] vectorAction)
        {
            character.Act(new Vector2(vectorAction[0], vectorAction[1]));
        }


        // Start is called before the first frame update
//        void Start()
//        {
//            //higher speed than pedestrian and police
//            belief = new float[envConfig.worldAgentNum, 2];
//            //init belief
//            ResetBelief();
//        }

//        private void ResetBelief()
//        {
//            for(int i = 0; i < belief.Length / 2; ++i)
//            {
//                belief[i, 0] = initialProb;
//                belief[i, 1] = 1 - belief[i, 0];
//            }
//        }
//
//        public void Act()
//        {
//            float max_0_belief = -1;
//            int max_0_index = -1;
//            for (int i = 0; i < belief.Length / 2; ++i)
//            {
//                if(belief[i, 0] >= max_0_belief)
//                {
//                    max_0_belief = belief[i, 0];
//                    max_0_index = i;
//                }
//            }
//
//            print("ThiefAgent: " + max_0_index);
//            if(max_0_belief <= 0f)
//            {
//                lastActionIndex = Random.Range(0, this.actions.Length / 2);
//            }
//            else
//            {
//                enemyIndex = max_0_index;
//                MonoBehaviour enemy = envConfig.GetAgent(enemyIndex);
//
//                float max_dist = 0f;
//                int bestActionIndex = -1;
//
//
//
//                for(int i = 0; i < this.actions.Length / 2; ++i)
//                {
//
//                    Vector3 forceDirection = new Vector3(actions[i, 0], 0, actions[i, 1]);
//                    Vector3 newVelocity = this.rigidBody.velocity + (forceDirection * Scale) / rigidBody.mass;
//
//                    Vector3 nextPosition = this.transform.position + newVelocity * 0.02f; //speed * time + position
//                    float distance = Vector3.Distance(nextPosition, enemy.transform.position);
//                    if (distance > max_dist)
//                    {
//                        max_dist = distance;
//                        bestActionIndex = i;
//                    }
//                }
//
//            }
//
//        }
//        void FixedUpdate()
//        {
//            Act();
//            //print(lastActionIndex);
//            Vector3 forceDirection = new Vector3(actions[lastActionIndex, 0], 0, actions[lastActionIndex, 1]);
//            rigidBody.AddForce(forceDirection * Scale);
//
//            Vector3 newPosition = Vector3.zero;
//            newPosition.x = transform.position.x > Config.LandSize ? -100f
//                : transform.position.x < 0 ? 100f : 0f;
//            newPosition.z = transform.position.z > Config.LandSize ? -100f
//                : transform.position.z < 0 ? 100f : 0f;
//
//            transform.position += newPosition;
//        }
    }
}
