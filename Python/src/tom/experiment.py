...
def train(arglist):
    ...
    for i, policy_agent in enumerate(env.world.policy_agents):
        polices.append(ToMAgent(policy_agent, env.world, scenario, new_obs_n[i], arglist.exp_name, use_gt_belief=arglist.use_gt_belief, use_distance=arglist.use_distance, direct_chase=arglist.direct_chase))
    if arglist.restore:
